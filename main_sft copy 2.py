...
# Local imports (assuming these are your custom modules)
from utils import *
...
if script_args.optimize_model == "True":
    ...
    for step in range(outer_epochs):
        ...
        for client in range(fed_args.num_clients):
            ...
            optimizer = AdamW(
                lora_params,
                lr=1e-4,               # Try 1e-3 or 1e-4, tune as needed
                weight_decay=0.001,     # Standard value for L2 regularization
                betas=(0.9, 0.999)     # Momentum (beta1=0.9, beta2=0.999)
            )
        ...
        global_path = os.path.join(f"{experiement_path}/global_step_{step}/{sft_adapter_name}")
