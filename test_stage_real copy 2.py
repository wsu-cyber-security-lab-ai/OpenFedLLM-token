...
class CustomDataCollator:
    ...
    def __call__(self, features):
        # print("Collator features type:", type(features))
        # print("Collator batch size:", len(features))
        # print("Sample feature keys/types:", [(k, type(v)) for k,v in features[0].items()] if len(features) > 0 else "Empty batch")
        
        # Separate tensor-compatible data and list-of-string data
        tensor_features = []
        forbidden_tokens_list = []
        required_tokens_list = []
        is_authorized_list = []

        for f in features:
            # Extract tensor-compatible fields for collator
            filtered = {k: v for k, v in f.items() if k not in ["forbidden_tokens", "required_tokens", "is_authorized"]}
            tensor_features.append(filtered)

            forbidden_tokens_list.append(f.get("forbidden_tokens", []))
            required_tokens_list.append(f.get("required_tokens", []))
            is_auth = f.get("is_authorized", 0)
            if isinstance(is_auth, torch.Tensor):
                is_auth = is_auth.item()
            is_authorized_list.append(is_auth)

        # Use default collator on tensor-compatible fields only
        batch = default_data_collator(tensor_features)

        # Add back custom fields without converting to tensors (except is_authorized)
        batch["forbidden_tokens"] = forbidden_tokens_list
        batch["required_tokens"] = required_tokens_list
        batch["is_authorized"] = torch.tensor(is_authorized_list)

        return batch


class LeakageAwareTrainer(SFTTrainer):
    def training_step(self, model, inputs, *args, **kwargs):
        loss = super().training_step(model, inputs, *args, **kwargs)

        # # Inspect gradients
        # print("Gradients for trainable parameters:")
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None, **kwargs):
        ...
        is_authorized = inputs.get("is_authorized")  # shape: (batch_size,)
        forbidden_tokens_batch = inputs.pop("forbidden_tokens")
        required_tokens_batch  = inputs.pop("required_tokens")
        ...
        print(f"Labels contributing to loss: {(labels != -100).sum(dim=1)}")
        ...
        # if not hasattr(self, '_printed_masking'):
        #     # Only print once per run to reduce log spam
        #     print("Labels (first sample slice):", labels[0][:50])
        #     print("Num non--100:", sum([l != -100 for l in labels[0].tolist()]))
        #     self._printed_masking = True
        ...
        predicted_strings = [
            tokenizer.decode([tok for tok in token_ids if tok != tokenizer.pad_token_id], skip_special_tokens=True).lower()
            for token_ids in predicted_tokens.cpu().tolist()
        ]

        # penalty_value = 0.0
        # batch_size, seq_len, vocab_size = logits.shape
        # for i in range(batch_size):
        #     forbidden = [ftok.lower() for ftok in forbidden_tokens_batch[i]]
        #     forbidden_token_ids = []
        #     for ftok in forbidden:
        #         forbidden_token_ids.extend(tokenizer.encode(ftok, add_special_tokens=False))
        #     if not forbidden_token_ids:
        #         continue
        #     # Only run for UNAUTHORIZED examples, optional
        #     if not is_authorized[i]:
        #         probs = torch.softmax(logits[i], dim=-1)  # shape: seq_len x vocab_size
        #         # For each forbidden token id, sum the probability across the sequence
        #         for tok_id in forbidden_token_ids:
        #             penalty_value += probs[:, tok_id].sum()


        batch_size, seq_len, vocab_size = logits.shape
        penalty = 0.0
        for i in range(batch_size):
            forbidden_tokens = forbidden_tokens_batch[i]
            forbidden_token_ids = set()
            for ftok in forbidden_tokens:
                forbidden_token_ids.update(tokenizer.encode(ftok, add_special_tokens=False))
            if not is_authorized[i]:
                for t in range(seq_len):
                    label_id = labels[i, t].item()
                    # Only penalize if the gold label is NOT forbidden (fix!)
                    if label_id == -100 or label_id in forbidden_token_ids:
                        continue
                    prob = torch.softmax(logits[i, t], dim=-1)
                    for ft_id in forbidden_token_ids:
                        penalty += prob[ft_id]


        penalty_tensor = torch.tensor(penalty).to(device)

        # Set penalty weight as needed
        penalty_weight = 1  # set to zero or small value to test
        total_loss = ce_loss + penalty_weight * penalty_tensor

        print(f"ce_loss: {ce_loss.item():.4f}, penalty: {penalty:.4f}, total_loss: {total_loss.item():.4f}")
...
if __name__ == "__main__":
    ...
    def tokenize_and_format(example):
        ...
        return {
            ...
            "is_authorized": int(example.get("is_gold", False)),  # Return int, not tensor
            "forbidden_tokens": example.get("forbidden_tokens", []),  # List[str]
            "required_tokens": example.get("required_tokens", []),    # List[str]
        }
        # return {
        #     "input_ids": input_ids,
        #     "attention_mask": attention_mask,
        #     "labels": labels,
        #     "is_authorized": torch.tensor(1 if example.get("is_gold", False) else 0),
        #     "forbidden_tokens": example.get("forbidden_tokens", []),
        #     "required_tokens":  example.get("required_tokens", []),
        # }
    ...
    valid_dataset = valid_dataset.map(tokenize_and_format)
    ...
    if not os.path.exists(SFT_MODEL_NAME):
        ...
        print("Initial weights (first 5 elems):", list(model.parameters())[0].view(-1)[:5].cpu().detach().numpy())
        ...
        # print("Trainable parameter count:", n_trainable)
        # for name, param in model.named_parameters():
        #     if param.requires_grad:
    ...
    for i in range(numOfRecords):
        ...
        with torch.no_grad():
            ...
            output = model.generate(
                ...
                max_new_tokens=200,           # longer than longest expected response
    ...
    def entity_level_reveal(generated, entities, is_gold):
        if is_gold:
    ...
    for i in range(numOfRecords):
        ...
        is_gold = orginal_dataset[i].get("is_gold", True)
        ...
        entity_correct = entity_level_reveal(generated, entities, is_gold)
    ...
    for org, stats in org_stats.items():
        ...
        for wrong in stats['wrong_details']:
            ...
            print("Is gold (should reveal):", wrong["is_gold"])
